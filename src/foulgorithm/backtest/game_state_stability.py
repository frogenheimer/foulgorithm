...
def slope_in(start, end):
    ev = d[(d["kickoff_utc"] >= pd.Timestamp(start, tz="UTC")) &
           (d["kickoff_utc"] < pd.Timestamp(end, tz="UTC"))]
    if ev.empty: return None, 0
    ...
    for _, b in ev.groupby(wk):
        ...
        for r in b.itertuples():
            ...
            xs.append(r.p_win - 0.5); ys.append(float(r.fouls_committed)/base)
    if len(xs) < 500: return None, len(xs)
    return float(np.polyfit(xs, ys, 1)[0]), len(xs)

print(f"{'window':<22}{'n':>9}{'slope':>10}")
print("-"*42)
for a, b in [("2018-01-01","2020-01-01"),("2020-01-01","2022-01-01"),
             ("2022-01-01","2024-01-01"),("2024-01-01","2026-09-01")]:
    sl, n = slope_in(a, b)
