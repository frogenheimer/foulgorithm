...
def tie(
    home, away, competition="FA Cup", kind="total", slug=None, kickoff="2026-09-01T19:00:00+00:00"
):
    ...
    return {
        ...
        "referee_display": "Andrew Kitchen",
...
def history():
    ...
    for i in range(30):
        rows.append(
            {
                **match("Arsenal", "Chelsea", hf=10, af=12, hy=2, ay=1),
        ...
        rows.append(
            {
                **match(
                    "Wrexham", "Burnley", hf=14, af=9, hy=3, ay=2, season="2026-27", division="E1"
                ),
...
class TestHouseSheetGating:
    ...
    def test_a_premier_league_tie_takes_the_house_sheet(self):
        payload = cups.build(
            [tie("Arsenal", "Chelsea", kind="full")],
            history(),
            {},
            {},
            sheets={"arsenal-v-chelsea-fa-cup": self.SHEET},
            now=NOW,
        )
    ...
    def test_a_championship_tie_refuses_one_even_when_handed_it(self):
        ...
        payload = cups.build(
            [tie("Arsenal", "Wrexham", kind="total")],
            history(),
            {},
            {},
            sheets={"arsenal-v-wrexham-fa-cup": self.SHEET},
            now=NOW,
        )
...
class TestLineups:
    ...
    def test_a_confirmed_eleven_reaches_its_tie(self):
        sheets = {
            "Arsenal|Arsenal v Wrexham": _lineup("4-3-3", ["Raya", "Saliba"]),
            "Wrexham|Arsenal v Wrexham": _lineup("4-4-2", ["Okonkwo", "O'Connor"]),
        }
        payload = cups.build(
            [tie("Arsenal", "Wrexham")], history(), {}, {}, lineups=sheets, now=NOW
        )
...
def _lineup(formation, starters):
    ...
    return SimpleNamespace(formation=formation, starters=starters, lines=[], bench=[])
...
class TestPlayers:
    ...
    def squads():
        ...
        def p(name, club, pos="M", mins=900, fouls=10):
            return PlayerStats(
                ...
                yellows=1,
                reds=0,
                fouls_per_90=round(fouls / (mins / 90), 2),
                fouls_won_per_90=0.5,
                tackles_per_90=0.7,
                minutes_by_division={"E0": mins},
            )
    ...
    def test_both_sides_get_an_eleven(self):
        payload = cups.build(
            [tie("Arsenal", "Wrexham")], history(), {}, {}, squads=self.squads(), now=NOW
        )
    ...
    def test_a_club_we_have_no_squad_for_is_honest_rather_than_empty(self):
        payload = cups.build(
            ...
            {},
            {},
            squads={"Arsenal": self.squads()["Arsenal"]},
            now=NOW,
        )
    ...
    def test_a_player_row_carries_no_probability(self):
        ...
        assert TestNoPlayerNumbers.carrying(payload["ties"][0]["players"], self.NO_PRICES) == []
...
class TestPitchShape:
    ...
    def test_both_sides_carry_lines(self):
        payload = cups.build(
            [tie("Arsenal", "Wrexham")], history(), {}, {}, squads=TestPlayers.squads(), now=NOW
        )
...
class TestFullSquad:
    ...
    def test_a_confirmed_starter_we_hold_nothing_on_joins_the_squad(self):
        ...
        def spot(n, pos):
            return SimpleNamespace(name=n, position=pos, shirt=None, detail="", captain=False)

        sheets = {
            "Arsenal|Arsenal v Wrexham": SimpleNamespace(
                formation=None,
                starters=["Youth Debutant"],
                spots=[spot("Youth Debutant", "M")],
                lines=[],
                bench=[],
            )
        }
